...
class FrameSession {
  ...
  _onFrameNavigated(framePayload, initial) {
    if (this._eventBelongsToStaleFrame(framePayload.id))
      return;
    if (process.env.PLAYWRIGHT_STEALTH_RUNTIME) { clearTimeout(this._stealthDisableTimer); this._client.send("Runtime.enable", {}).catch(() => {}); }
  ...
  _onExecutionContextCreated(contextPayload) {
    if (process.env.PLAYWRIGHT_STEALTH_RUNTIME) { clearTimeout(this._stealthDisableTimer); this._stealthDisableTimer = setTimeout(() => { this._client.send("Runtime.disable", {}).catch(() => {}); }, 10); }
